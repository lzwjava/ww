...
def rnn_cell_forward_tests(target):
    ...
    assert a_next_tmp.shape == (
        5,
        10,
    ), f"Wrong shape for a_next. Expected (5, 10) != {a_next_tmp.shape}"
    assert yt_pred_tmp.shape == (
        2,
        10,
    ), f"Wrong shape for yt_pred. Expected (2, 10) != {yt_pred_tmp.shape}"
    ...
    assert np.allclose(
        np.tanh(np.dot(parameters_tmp["Waa"], a_prev_tmp)), a_next_tmp
    ), "Problem 3 in a_next expression. Related to a_prev?"
    assert np.allclose(
        softmax(np.dot(parameters_tmp["Wya"], a_next_tmp)), yt_pred_tmp
    ), "Problem 3 in yt_pred expression. Related to a_next?"
...
def rnn_forward_test(target):
    ...
    assert a.shape == (
        ...
        m,
        T_x,
    ), f"Wrong shape for a. Expected: ({n_a, m, T_x}) != {a.shape}"
    assert y_pred.shape == (
        n_y,
        m,
        T_x,
...
def lstm_cell_forward_test(target):
    ...
    assert cache[4].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[4](ft). {cache[4].shape} != {(n_a, m)}"
    assert cache[5].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[5](it). {cache[5].shape} != {(n_a, m)}"
    assert cache[6].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[6](cct). {cache[6].shape} != {(n_a, m)}"
    assert cache[1].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[1](c_next). {cache[1].shape} != {(n_a, m)}"
    assert cache[7].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[7](ot). {cache[7].shape} != {(n_a, m)}"
    assert cache[0].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[0](a_next). {cache[0].shape} != {(n_a, m)}"
    assert cache[8].shape == (
        n_x,
        m,
    ), f"Wrong shape for cache[8](xt). {cache[8].shape} != {(n_x, m)}"
    assert cache[2].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[2](a_prev). {cache[2].shape} != {(n_a, m)}"
    assert cache[3].shape == (
        n_a,
        m,
    ), f"Wrong shape for cache[3](c_prev). {cache[3].shape} != {(n_a, m)}"

    assert a_next.shape == (
        n_a,
        m,
    ), f"Wrong shape for a_next. {a_next.shape} != {(n_a, m)}"
    assert c_next.shape == (
        n_a,
        m,
    ), f"Wrong shape for c_next. {c_next.shape} != {(n_a, m)}"
    assert y_pred.shape == (
        n_y,
        m,
    ), f"Wrong shape for y_pred. {y_pred.shape} != {(n_y, m)}"
    ...
    assert np.allclose(
        y_pred[1],
        [0.417098, 0.449528, 0.223159, 0.278376, 0.68453, 0.419221, 0.564025, 0.538475],
    ), "Wrong values for y_pred"
