...
def get_experiences(memory_buffer):
    ...
    states = torch.tensor(
        np.array([e.state for e in experiences if e is not None]), dtype=torch.float32
    )
    actions = torch.tensor(
        np.array([e.action for e in experiences if e is not None]), dtype=torch.float32
    )
    rewards = torch.tensor(
        np.array([e.reward for e in experiences if e is not None]), dtype=torch.float32
    )
...
def get_action(q_values, epsilon=0.0):
    ...
    Args:
        q_values (torch.Tensor):
            ...
            this Tensor should have a shape of [1, 4] and its elements should
...
def create_video(filename, env, q_network, fps=30):
    ...
    Args:
        ...
        q_network (torch.nn.Module):
            A PyTorch model that maps states to Q values.
        fps (int):
