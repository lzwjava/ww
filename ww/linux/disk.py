...
def create_disk_usage_chart():
    ...
    bars = plt.bar(
        range(len(filesystems)),
        used_percentages,
        color=[
            "red" if x > 80 else "orange" if x > 60 else "green"
...
def run():
    """Run disk usage command"""
    filesystems, used_percentages, mount_points = get_disk_usage()

    print(f"{'Filesystem':<30} {'Used %':<10} {'Mount Point'}")
    print("-" * 60)
    for fs, usage, mp in zip(filesystems, used_percentages, mount_points):
        print(f"{fs:<30} {usage:<10} {mp}")
